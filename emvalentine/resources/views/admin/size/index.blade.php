...
@section('centro')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">Tallas</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="text-dark" href="{{ url('/admin') }}">Inicio</a></li>
                <li class="breadcrumb-item active">Listado de tallas</li>
            </ol>
        </div>
    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row m-t-20">
                        <div class="col-6">
                            <div class="form-group">
                                <a href="{{ url('/admin/size/create') }}" class="btn waves-effect waves-light btn-success btn-sm text-white createCategorieIndex btn-rounded">Nueva Talla</a>
                            </div>
                        </div>
                    ...
                    <table id="demo-foo-addrow2" class="table table-bordered table-hover toggle-circle" data-page-size="5">
                        <thead>
                            <tr>
                                <th data-sort-initial="true" data-toggle="true"> Talla </th>
                        ...
                        <tbody id="tdBodyCategorie">
                            @foreach ($sizes as $size)
                                <tr id="SizeTR{{ $size->id }}">
                                    <td>{{ $size->name }}</td> 
                                    <td>
                                        <a href="{{ url("/admin/size/$size->id/edit") }}" style="margin: 0px 10px"><i class="icon icon-pencil text-inverse m-r-10" ></i> </a>
                                         <a href="javascript:void(0)" data-id="{{$size->id}}" id="DeleteSize"> <i class="icon icon-close text-danger"></i> </a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    $(document).on('click','#DeleteSize',function() 
    {
        ...
        $.ajax({
            ...
            success: function(data) 
            {
                $('#SizeTR'+ id).remove();
                $.toast({
                    heading: 'Talla eliminada correctamente..',
                    position: 'top-right',
                    loaderBg:'#9bde2c',
                    icon: 'success',
                    hideAfter: 2500,
